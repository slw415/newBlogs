...
@section('content')
    ...
    <ul class="nav nav-pills"id="first_ul">
        ...
        <li class="nav-item">
            <a class="nav-link disabled" href="{{url('/admin/message')}}">导航栏列表</a>
        </li>
    </ul>
        <div class="mt-2 ml-2 ">
    <form class="form-inline" method="get" action="{{url('/admin/message')}}">
        {{csrf_field()}}
        <input type="text" name="input" class="form-control "style="width: 350px;" id="user" placeholder="输入评论内容和文章标题"value="{{isset($input)?$input:''}}">
