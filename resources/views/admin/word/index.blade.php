...
@section('content')
    ...
    <ul class="nav nav-pills"id="first_ul">
        ...
        <li class="nav-item">
            <a class="nav-link disabled" href="{{url('/admin/words')}}">留言列表</a>
        </li>
    </ul>
        <div class="mt-2 ml-2 ">
    <form class="form-inline" method="get" action="{{url('/admin/words')}}">
        {{csrf_field()}}
        <input type="text" name="input" class="form-control "style="width: 350px;" id="user" placeholder="输入留言内容或者作者"value="{{isset($input)?$input:''}}">
    ...
    <script>
        ...
        $('#list{{$v1['con']['id']}}').find('td .del').click(function () {
            layer.confirm('您确定要删除这条记录？', {
                ...
                $.post('{{ url('/admin/words/'.$v1['con']['id']) }}', {
                    '_method': 'delete',
                    '_token': "{{ csrf_token() }}",
                    'id': "{{ $v1['con']['id'] }}",
                    'pid': "{{ $v1['con']['pid'] }}"
